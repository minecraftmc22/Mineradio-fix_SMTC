...
const BEATMAP_CACHE_DIR = process.env.MINERADIO_BEAT_CACHE_DIR || path.join(os.tmpdir(), 'Better-MineradioCache', 'beatmaps');
...
function normalizeManifestUpdateInfo(data) {
  ...
  const patchInfo = patch && patch.downloadUrl ? {
    name: patch.name || updateAssetNameFromUrl(patch.downloadUrl) || `Better-Mineradio-${APP_VERSION}→${latestVersion}.patch.json`,
  ...
  const assetInfo = downloadUrl ? {
    name: asset.name || updateAssetNameFromUrl(downloadUrl) || `Better-Mineradio-${latestVersion}-Setup.exe`,
  ...
  return {
    ...
    release: {
      ...
      name: release.name || data.name || ('Better-Mineradio v' + latestVersion),
...
async function readUpdateManifest(ref) {
  ...
  if (/^https?:\/\//i.test(value)) {
    const resp = await fetch(value, {
      headers: { 'User-Agent': `Better-Mineradio/${APP_VERSION}` },
    });
...
function localUpdateFallback(reason, opts) {
  ...
  return {
    ...
    release: {
      ...
      name: 'Better-Mineradio v' + APP_VERSION,
      version: APP_VERSION,
      htmlUrl: '',
      downloadUrl: '',
...
function parseLatestYmlUpdateInfo(text, reason) {
  ...
  const assetPath = yamlScalar(text, 'path') || yamlScalar(text, 'url') || `Better-Mineradio-${latestVersion}-Setup.exe`;
  ...
  return {
    ...
    release: {
      ...
      name: 'Better-Mineradio v' + latestVersion,
      version: latestVersion,
...
async function fetchLatestUpdateInfo() {
  ...
  try {
    const resp = await fetch(apiUrl, {
      ...
      headers: {
        'User-Agent': `Better-Mineradio/${APP_VERSION}`,
    ...
    return {
      ...
      release: {
        ...
        name: data.name || ('Better-Mineradio v' + latestVersion),
        version: latestVersion,
...
function safeUpdateFileName(name, version) {
  const raw = String(name || '').trim() || `Better-Mineradio-${version || APP_VERSION}.exe`;
  const cleaned = raw
    .replace(/[<>:"/\\|?*\x00-\x1F]/g, '-')
    .replace(/\s+/g, ' ')
    .trim()
    .slice(0, 160);
  return cleaned || `Better-Mineradio-${version || APP_VERSION}.exe`;
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (pn === '/api/app/version') {
    sendJSON(res, {
      name: APP_PACKAGE.name || 'better-mineradio',
      productName: APP_PACKAGE.productName || 'Better-Mineradio',
      version: APP_VERSION,
      update: {
