...
![Better-Mineradio 暗场启动页](./docs/assets/readme/cinema-beat-smoke.png)

> 本项目是 [XxHuberrr/Mineradio](https://github.com/XxHuberrr/Mineradio) 的 Fork，由 [Minecraftmc22](https://github.com/Minecraftmc22) 维护，更名为 **Better-Mineradio**。
>
> 在原版基础上增加了 **SMTC（System Media Transport Controls）** 支持、**自定义歌词字体** 和 **标题模式设置**。

Better-Mineradio 是一款 Windows 桌面沉浸式音乐播放器，把天气电台、搜索播放、歌词舞台、粒子视觉和 3D 歌单架组合成一个更接近现场感的私人音乐空间。
...
在「设置 → 歌词 → 字体与字距」中新增「自定义」按钮，可输入任意 CSS 字体名（如 `Microsoft YaHei`、`SimSun`、`Consolas` 等）。设置自动保存到 localStorage，桌面歌词同步继承。

### 标题模式设置

在「设置 → 高级 → 标题设置」中新增 5 种窗口标题显示模式，所有模式均附加 `-Better-Mineradio` 后缀：

| 模式 | 格式 | 示例 |
| --- | --- | --- |
| 歌曲名 | `歌曲名 -Better-Mineradio` | `晴天 -Better-Mineradio` |
| 歌曲名-歌手 | `歌曲名-歌手 -Better-Mineradio` | `晴天-周杰伦 -Better-Mineradio` |
| 整句歌词 | `当前歌词行 -Better-Mineradio` | `故事的小黄花 -Better-Mineradio` |
| 逐词歌词 | `当前高亮部分 -Better-Mineradio` | `故事的小 -Better-Mineradio` |
| 软件名 | `Better-Mineradio` | `Better-Mineradio` |

逐词歌词模式随歌词高亮进度实时累进显示（200ms 节流），切歌和清空队列时自动更新。
...
Better-Mineradio 会请求 GitHub Releases latest 检测新版本。远端版本高于本地版本时，应用内更新入口会展示 Release 内容、下载安装包到本机用户数据目录，并通过系统打开安装包。

本地验证更新链路时，可以通过 `MINERADIO_UPDATE_MANIFEST` 指向一个本地 manifest JSON 或 HTTP 地址来模拟线上 Release。

## 第三方音乐平台说明

Better-Mineradio 不是网易云音乐、QQ 音乐或腾讯音乐娱乐集团的官方客户端，也不隶属于任何音乐平台。
